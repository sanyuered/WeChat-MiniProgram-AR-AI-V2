...
// 字体地址
const fontUrl = 'https://m.sanyue.red/demo/fonts/helvetiker_regular.typeface.json'
// 字体
var fontFace;
...
function setTextPosition(model1, model2) {
    // 右侧文字
...
function createText(message, colorIndex) {
    if (!fontFace) {
        return
    }
    // 颜色范围
    ...
    const shapes = fontFace.generateShapes(message, 100);
    const geometry = new THREE.ShapeGeometry(shapes);
    geometry.computeBoundingBox();
    const xMid = -0.5 * (geometry.boundingBox.max.x - geometry.boundingBox.min.x);
    geometry.translate(xMid, 0, 0);
    const model = new THREE.Mesh(geometry, matLite);
    // 字体缩放
...
// 加载文字模型（只支持英文字母）
function loadTextModel(callback) {
