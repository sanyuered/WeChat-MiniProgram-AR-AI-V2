...
// 机器人模型，带动画。
const robotUrl = 'https://m.sanyue.red/demo/gltf/road_sign.glb';
// webgl画面录制器的帧数
    ...
    wx.createSelectorQuery()
      ...
      .exec(res => {
        ...
        cameraBusiness.loadModel(robotUrl, function (model, animations) {
          ...
          // 加载字体
          cameraBusiness.loadTextModel(function () {
            // 创建文字，用作路标模型上
            const model1 = cameraBusiness.createText("Exit",0)
            const model2 = cameraBusiness.createText("Enter",1)
            // 设置文字位置
            cameraBusiness.setTextPosition(model1, model2)
          })
